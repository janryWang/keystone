...
function List(key, options) {
	...
	this.options = utils.options({//给options默认参数
		schema: {
...
List.prototype.register = function() {
	...
	//给List的Schema定义一个虚拟属性_,同时给其设置一个getter,其上下文为document,表示获取underscore的所有方法
	...
	//给每个document注册方法,在views/item.js中如果存在数据操作的话，会调用该方法，同时将相关参数传过来
